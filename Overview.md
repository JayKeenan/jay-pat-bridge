# Jay–Pat — Overview

The live card in short. Detail is in the other files. If this page and Jay disagree, ask — then fix law first.

**Stay safe → major → else NT → then how high. Slam later.** Minors are a tool.

Not describing your hand. We want our **best score**, and to get in their way cheaply. Before you bid: **what are we trying to play, and can we still stop there?** If no, pick another call.

Check [Updates](Updates.md) if you haven’t opened the folder in a while.

---

## How high (starting points)

Facing a *minimum*. Shape and controls can move you **one** step later. They do not skip quit → game.

| After | Quit | Invite | Game / force |
|-------|------|--------|----------------|
| **1NT (11–14)** | **0–10.** Pass, transfer-and-pass, or **crawl** (4M). **10 ≠ 2♠.** | **11–12 no 4M → 2♠ only** (ALERT). 4M → Stayman then invite | **13+.** 3NT or transfer/Stayman then game |
| **1♥ / 1♠** | **0–7.** Pass or 1NT then 2M | **11–12.** 3♦ = 4-card limit (ALERT); 1NT then 3M with three | **13+.** 2/1 / Jacoby. **8–10 = 2M**, not 4M, not 1♠ |
| **1♣ / 1♦** | 0–5 pass; 6–10 1NT (no 4-card major) | 11–12: 2NT or inverted 2m (ALERT) | **13+.** 2/1 or 3NT |
| **They rebid 1NT (15–17)** — `1m–1M` **or** `1♥–1♠` | **6–7 pass** (or 2M / 2♥ to play) | **8–9** 2♣ NMF (ALERT) | **10+** 2♦ GF or 3NT |
| **They rebid 2NT (18–19 / 18–20)** | Min **pass**, or **3M** to play | — | **3NT** or **4M**. No transfers |
| **2NT opening (20–21)** | **0–3.** Pass, or transfer-and-pass (5+M) | — | **4+.** 3NT or Stayman / transfer then game |

**After 2♣:** game force. **Pass 2NT** (22–24) with a bust. Do not pass a suit rebid. **20–21** opened **2NT**, not 2♣.

**Raise with a raise.** `♠AT753 ♥Q85 ♦K87 ♣65` after 1♥ is **2♥**, not 1♠. Over 1♠ the same 8-count raises 2♠ (2♥ would be GF). Same raise both ways.

---

## Announce (say it)

| Bid | Say |
|-----|-----|
| 1NT opening | “Good eleven to fourteen” |
| 2♦ / 2♥ over our 1NT | “Transfer to hearts / spades” |
| 3♦ / 3♥ over our 2NT | “Transfer to hearts / spades” |
| 1NT over 1♥ or 1♠ | “Semi-forcing” |

## Alert (tap; explain if they ask)

| Bid | Say if asked |
|-----|----------------|
| 2♠ over our 1NT | “Clubs or invite” |
| 2NT over our 1NT | “Transfer to diamonds.” 3♦ = fit (3+ or Qx+) **or** max **or both**; else 3♣ |
| 2♥ / 2♠ after Stayman got 2♦ | “Weak, crawl” |
| 1♥ / 1♠ – 2♣ (we have not passed) | “Game force, not necessarily clubs” |
| 2♣ opening | “Strong, artificial” |
| 2♦ over our 2♣ | “Waiting” |
| 2♣ / 2♦ inverted | “Limit raise or better” |
| 2♥ / 2♠ over our 1♣ or 1♦ | “Weak jump shift” |
| 2♣ / 2♦ after our 15–17 1NT rebid (`1m–1M` or `1♥–1♠`) | “Two-way: 2♣ invite, 2♦ game force” |
| 2NT over 1M | “Jacoby — forcing raise” |
| 3♦ over 1M | “Four-card limit raise” |
| 3♥ over 1♠ | “Invitational, long hearts” |
| 2♣ after we passed | “Drury” |
| Splinter | “Shortness, game force” |

Do **not** alert 1♣, **2♦ / 2♥ 2/1** (five-card GF, on the card), or weak twos (including 2♦). **Do** alert 1M–2♣, the **2♣** opening, **2♦** waiting, and **1m–2M** (weak jump).

---

More detail: [Notrump](Notrump.md) · [Majors](Majors.md) · [Minors](Minors.md) · [Weak-Twos](Weak-Twos.md) · [Strong-2-Club](Strong-2-Club.md) · [2-Notrump](2-Notrump.md). Practice: [Cheat-Sheet](Cheat-Sheet.md). War is not locked. Ideas: [Doubles](Doubles.md) (**not law**).
